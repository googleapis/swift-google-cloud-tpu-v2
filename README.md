# Google Cloud Client Libraries for Swift - Cloud TPU API
...
```bash
swift package add-dependency https://github.com/googleapis/swift-google-cloud-tpu-v2.git --from 0.3.0
```

Then add `GoogleCloudTpuV2` to your target's dependencies:

```bash
swift package add-target-dependency GoogleCloudTpuV2 <target-name> --package swift-google-cloud-tpu-v2
```
